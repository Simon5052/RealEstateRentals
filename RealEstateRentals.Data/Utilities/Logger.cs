...
namespace RealEstateRentals.Data.Utilities
{
    public class Logger
    {
        ...
        public Logger()
        {
           var _logDirectories = new LogDirectory
            {
                Error = "./Error_Test",
                Info = "./Info_Test",
        ...
        {
           var _logDirectories = options.Value;
            SetDirectories(_logDirectories);
        }
        ...
        public void logError(Exception x)
        {
            try
            {
                ...
                var logFileName = string.Format("RealEstateRentals_Error_{0}_{1}.log", DateTime.Now.ToString("yyyy_MM_dd"), DateTime.Now.ToString("HH"));
        ...
        public void logInfo(string message)
        {
            try
            {
                ...
                var logFileName = string.Format("RealEstateRentals_Info_{0}_{1}.log", DateTime.Now.ToString("yyyy_MM_dd"), DateTime.Now.ToString("HH"));
                var logFilePath = Path.Combine(dir, logFileName);
        ...
        public void logWarning(string message)
        {
            lock (Locker)
            {
                try
                {
                    ...
                    var logFileName = string.Format("RealEstateRentals_Warning_{0}_{1}.log", DateTime.Now.ToString("yyyy_MM_dd"), DateTime.Now.ToString("HH"));
                    var logFilePath = Path.Combine(dir, logFileName);
